...
const Form = () => {
  ...
  const [celed, setCeled] = useState("")
    ...
    useEffect(() => {
      const sub = projectFirestore.collection("cardSafe").onSnapshot((snap) => {
          if (snap.empty) {
              setError("")
              setAllAnswers([])
              // setOldCards([])
    ...
    const pocetKaretFun = (event) => {
     ...
     {/* <button
        ...
        class="inline-flex bg-black text-white items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2 w-46"
      >Použít starou kartičku</button>
      <Modal
      ...
      >
        <div className='flex flex-col justify-center w-full items-center'>
          <span className='flex text-center text-3xl font-bold'>Staré kartičky</span>
          </div>
          <span className='flex flex-col gap-36'>
          {allAnswers.slice(0, 3).map((oneCard) => (
